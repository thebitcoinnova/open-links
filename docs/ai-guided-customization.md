...
15. When content changes affect remote images or generated presentation assets, refresh all committed content outputs in the same change batch:
...
- `docs/data-model.md` (`links[].referral` section)
...
The refresh command is the mutating step; review and commit its allow-listed outputs. Build and strict build consume committed content without modifying tracked files.
