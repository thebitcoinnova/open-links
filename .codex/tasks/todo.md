...
## task-social-profile-rename-resilience | 2026-07-23 00:00 CDT | Recover CI and make social-profile renames first-class

- [x] Diagnose the recent CI, nightly history, screenshot, upstream-sync, and Bright Builds auto-update failures.
- [x] Sync Staci's main branch with current upstream while preserving every fork-owned path.
- [x] Add strict X oEmbed compatibility for the current provider label and post-link wording.
- [x] Prevent public cache data from crossing source identities and retain complete same-source last-known-good metadata on regressions.
- [x] Add cache identity validation with rename-workflow remediation.
- [x] Add the deterministic `social:profile:rename` CLI, repo skill, generated skill metadata, CRUD routing, README discovery, and regression tests.
- [x] Prepare and verify the targeted upstream shared-fix PR without fork-owned paths.
- [x] Apply the shared fix to Staci's fork, refresh the renamed X profile, preserve historical rows byte-for-byte, and recover Altair through same-source fallback.
- [x] Repair and rerun Bright Builds Auto Update, then run and watch the affected GitHub workflows.
- [x] Run all required local verification, review the final diff, and record completion evidence and residual risks.

### Completion Review

- Result: Added strict current/legacy X oEmbed parsing, semantic X profile-bio capture, source-identity-isolated public caching, same-source `metadata_regression` fallback, cache-identity validation, and target-scoped X audience metrics. Added the guarded `social:profile:rename` CLI plus the `rename-social-profile` skill, generated skill metadata, README/CRUD routing, and regression coverage. Upstream PR `pRizz/open-links#68` was built from `upstream/main`, excludes every fork-owned path, and has passing Required Checks. The previously planned audit-resolver change was already merged upstream as `pRizz/open-links#67`, so no duplicate PR was opened.
...
## task-repair-ci-committed-content | 2026-08-26 21:53 CDT | Repair fresh-clone CI and Bright Builds failures

- [x] Add a clean-repository regression test proving every refresh-owned generated output can be discovered and staged.
- [x] Align Git ignore and nightly staging behavior with the committed-content contract.
- [x] Migrate the fork-owned lesson ledger and record its required audit baseline.
- [x] Refresh and stage the fork-owned generated content without changing profile, links, or site data.
- [x] Run focused checks, mandatory verification, clean-checkout deploy validation, and final diff review.

### Completion Review

- Result: Fresh clones can now discover, track, and stage every allow-listed committed-content output. The shared remote-cache fetcher no longer sends validators when the corresponding cached bytes are missing, so cold image-cache recovery fetches complete bodies instead of failing on unusable HTTP 304 responses. The fork now contains refreshed rich metadata, enrichment evidence, social previews, and recovered content-image assets while `data/profile.json`, `data/links.json`, and `data/site.json` remain byte-identical. The repository lesson ledger was migrated to 20 stable four-field blocks, and the required combined lesson-audit baseline was recorded without consolidation or archival.
- Verification: The staging regression failed with 12 missing staged paths before the fix and passed afterward. The missing-cache regression failed with `error` before the fix and passed with a fetched body afterward. `bun run content:refresh`, `bun run validate:data`, `bun scripts/bright-builds-check.ts all`, 38 focused tests, the 184-test deploy suite, `bun run biome:check`, `bun run studio:lint`, `bun run typecheck`, `bun run studio:typecheck`, Studio API tests, Studio integration tests, and `bun run ci:required` passed. A disposable clean commit containing the full repair passed `bun install --frozen-lockfile`, Bright Builds checks, and `CI=true bun run deploy:build`.
- Residual risk: The avatar source still returns HTTP 404 and therefore relies on the existing committed cached avatar. Standard validation retains the known manual Amazon Music metadata fallback plus top-level `$schema` warnings, and quality checks retain the existing SEO, focus-visible, and bundle-budget warnings. The shared staging/cache fixes should be proposed upstream in a targeted PR that excludes this fork's owned content paths.
