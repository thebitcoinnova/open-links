...
if [[ "$*" == "run quality:strict:ci" ]]; then
  echo "quality:strict:ci should have been skipped" >&2
  exit 99
fi
exit 0
`,
  ...
  assert.match(combinedOutput, /quality:strict:ci.*skipped/u);
  assert.match(combinedOutput, /failures=build:strict/u);
});

test("run-strict invokes CI advisory quality command after a successful build", (t) => {
  // Arrange
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "open-links-ci-test-"));
  const fakeBinDir = path.join(tempDir, "bin");
  const invocationLogPath = path.join(tempDir, "bun-invocations.log");
...
`,
  ...
  assert.deepEqual(invocations, ["run build:strict", "run quality:strict:ci"]);
